...
class StudentFlowApi {
  ...
  getLectureStudentView(serviceId: string, lectureCardIds: string[], courseLectureIds: string[], preCourseLectureIds: string[]) {
    return axiosApi.get<any>(this.baseUrl + `/studentInfoView?serviceId=${serviceId}&lectureCardIds=${lectureCardIds}&courseLectureIds=${courseLectureIds}&preCourseLectureIds=${preCourseLectureIds}`)
      .then(response => response && response.data || null);
  }
}
