...
class LectureFlowApi {
  ...
  findAllRecommendLectures(lectureRdo: LectureRdoModel) {
    ...
    return axiosApi
      .get<RecommendLectureListRdo>(this.baseUrl + '/recommend', { params })
      .then(
        response =>
          (response && new RecommendLectureListRdo(response.data)) ||
          new RecommendLectureListRdo()
      );
  }
  ...
  findCollegeLectureCount() {
    //
    return axiosApi
      .get<CollegeLectureCountRdo[]>(this.baseUrl + '/college')
      .then(
        response =>
          (response &&
            Array.isArray(response.data) &&
            response.data.map(collegeLectureCount => collegeLectureCount)) ||
          []
      );
  }
  ...
  findAllRqdTableViews(lectureFilterRdoV2: LectureFilterRdoModelV2) {
    return axiosApi
      .post<OffsetElementList<LectureTableViewModel>>(
        `${this.baseUrl}/required/view`,
        lectureFilterRdoV2
      )
  ...
  findAllFilterCountViews(lectureFilterRdoV2: LectureFilterRdoModelV2) {
    return axiosApi
      .post(`${this.baseUrl}/required/filter/count`, lectureFilterRdoV2)
      .then(response => (response && response.data) || null)
      .catch(err => err && null);
  }
