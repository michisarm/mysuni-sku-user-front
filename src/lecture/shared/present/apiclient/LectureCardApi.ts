...
class LectureCardApi {
  ...
  baseUrl =
    ...
    process.env.REACT_APP_LECTURE_CARD_API === undefined ||
    process.env.REACT_APP_LECTURE_CARD_API === ''
      ? '/api/lecture/lectureCards'
      : process.env.REACT_APP_LECTURE_CARD_API;
  ...
  findAllLectureCards(
    ...
    return axiosApi
      .get<LectureCardModel[]>(this.baseUrl, { params })
  ...
  findLectureCard(lectureCardId: string) {
    //
    return axiosApi
      .get<LectureCardModel>(this.baseUrl + `/${lectureCardId}`)
