  ...
  removeLectureFromStorage(serviceId: string) {
    ...
    if (savedPopularLearningList && savedPopularLearningList.length > 0) {
      ...
      if (
        PopularMain &&
        PopularMain.results &&
        PopularMain.results.length > 0
      ) {
        PopularMain.results = PopularMain.results.filter(
