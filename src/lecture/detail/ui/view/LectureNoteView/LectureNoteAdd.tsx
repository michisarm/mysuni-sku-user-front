...
import { getLectureNoteWriteState, setLectureNoteTab, setLectureNoteWriteState } from '../../../store/LectureNoteStore'
...
const LectureNoteAdd: React.FC<Props> = function LectureNoteView({
  ...
  const onClickTextArea = useCallback(() => {
    ...
    if (noteType === 'default') {
      if (cubeType === 'Video') {
        ...
        if(panoptoState) {
          onChangeTime(panoptoState!.currentTime ? panoptoState!.currentTime : 0)
        }
  ...
  const handelTextArea = useCallback((e: any) => {
    onChange({
        ...
        !editorVisibleFlag && (
          <div className="note_input">
            <Input
              ...
              placeholder="강의에 대한 Note를 작성할 수 있습니다."
            />
          </div>
        ) 
      }
          ...
          <>
            <div className="mynote mynote_write">
              <div className="note_info">
                {
                  ...
                  <span className="time">
                    ...
                    <span className="iconrefresh" onClick={timeReset}><Image src={`${process.env.PUBLIC_URL}/images/all/btn-time-refresh.svg`} alt="새로고침"/></span> 
                  </span>
                  )
                }
                {
              ...
              <div className="note_btn">
                <Button className="cancel" onClick={() => onHandleCancel()}>취소</Button>
                <Button className="save" onClick={() => onSave()}>저장</Button>
                <span className="txt_cnt"><span className="txt_now">{addNote.content.length}</span>/<span>1000</span></span>
              </div>
            </div>
          </>
        ) 
