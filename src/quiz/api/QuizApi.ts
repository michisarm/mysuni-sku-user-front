...
export function deleteQuiz(
  ...
  // 리턴타입 변경하기
  return axios
    .delete(`${BASE_URL}/${quizId}`)
}

// 퀴즈 수정 
export function modifyQuiz(
  quizId: string, changeValue: QuizTableList
): Promise<any> {
...
export function registerAnswer(
  ...
  // 리턴타입 변경하기
  return axios
    .post<string>(`${BASE_URL}`, answerInfo)
    .then((response) => response?.data);
}

// 정답 수정
export function modifyAnswer(
  quizId: string, changeAnswerInfo: any
): Promise<any> {
...
export function findAllAnswer(
  quizId: string, limit: number, offset: number,
): Promise<string> {
...
export function findAnswer(
  quizId: string, memberId: string
): Promise<any> {
  // /quizQuestionAnswer/{quizQuestionId}/{memberId}
  // 리턴타입 변경하기
  return axios
    .get<any>(`${ANSWER_URL}/${quizId}/${memberId}`)
    .then(response => response?.data)
}
