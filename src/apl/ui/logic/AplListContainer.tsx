...
@observer
@reactAutobind
class AplListContainer extends React.Component<Props, States> {
  //
  ...
  init() {
    ...
    */
    ...
    this.onChangeAplQueryProps(
      `dateOptions`,
      SelectType.dateOptions[0].value
    );
  ...
  */
  ...
  modifyApl(isUse: boolean, aplId: number) {
    ...
    if (aplService) {
      aplService
        .modifyApl(isUse, aplId)
        .then(() => this.reRetrieve());
    }
  }
          ...
          {totalCount === 0 ? null : (
            <>
              <div className="center">
                <Pagination
                  activePage={
                    pageMap.get('apl') ? pageMap.get('apl').page : 1
                  }
                  totalPages={
                    pageMap.get('apl')
                      ? pageMap.get('apl').totalPages
                      : 1
                  }
                  onPageChange={(e, data) =>
