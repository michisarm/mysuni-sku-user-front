...
function MyLearningListContainerV2(props: Props) {
  ...
  console.log('resultEmpty ::', resultEmpty);
  ...
  const fetchModelsByContentType = async (contentType: MyLearningContentType) => {
    ...
    switch (contentType) {
      ...
      case MyLearningContentType.Completed: {
        myTrainingService!.changeFilterRdoWithServiceType(viewType);
  ...
  const fetchModelsByConditions = async (serviceType?: string) => {
    if (serviceType) {
      myTrainingService!.changeFilterRdoWithServiceType(serviceType);
    }
  ...
  const onClickSort = (column: string, direction: string) => {
    myTrainingService!.sortByDirection(column, direction);
  ...
  const renderNoSuchContentPanel = (contentType: MyLearningContentType) => {
    const message = NoSuchContentPanelMessages.getByContentType(contentType);

    // link 는 InProgress 일 경우에만 존재한다.
  ...
  const renderNoSuchFilterResult = () => {
    const message = '필터 조건에 해당하는 결과가 없습니다.';
