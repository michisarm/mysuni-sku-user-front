...
import {
  ...
  createGroupMembers,
} from 'community/service/useGroupMemberList/useGroupMember';
...
const AdminGroupCreateView: React.FC<AdminGroupCreateViewProps> = function AdminMemberView({
  ...
  const AllData =
    communityGroupMembers &&
    communityGroupMembers.results.map(item => item.memberId);
  ...
  const routeToGroupDetail = useCallback(
    (groupId: string) => {
      history.push(
        `/community/admin/${communityId}/memberManagement/group/Detail/${groupId}`
      );
    },
    [communityId]
  );
  ...
  const saveGroupInformation = useCallback(async () => {
    if (!name || name.length === 0) {
      reactAlert({
        title: '필수 정보 입력 안내',
        message:
          '‘그룹명’은 필수 항목입니다.<br />해당 정보를 입력하신 후 저장해주세요.',
      });
      return true;
      ...
      reactAlert({
        title: '필수 정보 입력 안내',
        message:
          '‘그룹 설명’은 필수 항목입니다.<br />해당 정보를 입력하신 후 저장해주세요.',
      });
      return true;
    }
    ...
    if (!groupId) {
      ...
      reactAlert({
        title: '완료 안내',
        message: '그룹 등록 처리가 완료되었습니다.',
      });
    } else {
      await updateGroup();
      reactAlert({
        title: '완료 안내',
        message: '그룹 수정 처리가 완료되었습니다.',
      });
    }
  ...
  const removeMemberGroup = useCallback(async () => {
    ...
    const checkManager =
      selectedList &&
      selectedList.find(item => item === adminGroupCreateItem.managerId);
    ...
    reactConfirm({
      ...
      onOk: async () => {
        groupId &&
          (await deleteGroupMembers(communityId, groupId, selectedList));
        setSelectedList([]);
        getAdminGroup();
        getAllGroupMemberByQuery();
        // routeToGroupList();
  ...
  const approveAdmin = useCallback(async () => {
    if (selectedList.length == 1) {
      ...
      reactAlert({
        title: '알림',
        message: '그룹장으로 지정할 멤버를 선택해주세요.',
      });
    } else {
      reactAlert({
        title: '알림',
        message: '그룹장은 1명만 지정할 수 있습니다.',
      });
    }
        ...
        <>
          ...
          <div className="table-board-title">
            <div className="table_list_string">
              ㆍ전체{' '}
              <strong>
                {communityGroupMembers && communityGroupMembers.totalCount}
              </strong>
              그룹멤버
            </div>
            <div className="right-wrap">
              ...
              {/*TODO : modal 기능 분리 필요 */}
              <Modal size="large" open={open} className="base w1000">
                <Modal.Header>
                  <span>그룹 멤버 추가</span>
                  <button
                    className="admin_popup_close"
                ...
                <Modal.Content className="admin_popup_add">
                  ...
                  {communityId &&
                    AdminMemberPage(communityId, 'APPROVED', groupId)}
                </Modal.Content>
              ...
              <button
                className="ui button admin_table_button02"
                onClick={() => removeMemberGroup()}
              >
                그룹 멤버 삭제
              </button>
            </div>
          ...
          {communityGroupMembers &&
          communityGroupMembers?.results.length > 0 ? (
            <>
                    ...
                    <th>
                      <Checkbox
                        ...
                        checked={
                          ...
                          selectedList.length ===
                            communityGroupMembers.results.length
                        }
                ...
                <tbody>
                  {communityGroupMembers?.results.map((item, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        {communityGroupMembers?.totalCount -
                          index -
                          (activePage - 1) * limit}
                      </td>
