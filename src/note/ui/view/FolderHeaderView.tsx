...
const FolderHeaderView: React.FC<FolderHeaderViewProps> =
  function FolderHeaderView({ noteList, folder, folderNoteCount }) {
    ...
    const [editFolderName, setEditFolderName] =
      useState<string>(firstFolderName);
    const [editFolderOriginName, setEditFolderOriginName] =
      useState<string>(firstFolderName);
    ...
    const changeArrayOrder = useCallback(
      (list: Folder, targetIdx: number, moveValue: number) => {
        ...
        if (newPosition < 0 || newPosition >= list.folders.idNames.length)
          return;
          ...
          <Segment className="full">
            ...
            {noteList && noteList.results.length === 0 && (
              <div className="note_nodata">
                <Icon>
                  <Image
                    src={`${PUBLIC_URL}/images/all/no-contents-80-px.svg`}
                  />
                </Icon>
                <p
                  className="txt"
