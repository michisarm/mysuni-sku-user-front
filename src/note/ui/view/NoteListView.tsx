                  ...
                  {subNoteList && subNoteList.map((subNoteItem, subIndex) => (
                    ...
                    subNoteItem.noteWithLectureList.results.map((subItem, subIndex) => (
                      <div key={subIndex} className={`mynote ${noteUdoItem?.index === subIndex && 'mynote_write'}`} >
                        <div className="note_info">
                          {(subItem.note.playTime && subItem.note.playTime.indexOf('Note ') === -1) && (subItem.note.cubeType === 'Video' || subItem.note.cubeType === 'Audio') &&
                            (
                          ...
                          {((!subItem.note.playTime || subItem.note.playTime.indexOf('Note ') > -1) || (subItem.note.cubeType !== 'Video' && subItem.note.cubeType !== 'Audio')) && <Icon><Image src={`${PUBLIC_URL}/images/all/btn-lms-note-14-px.svg`} alt="노트이미지" /></Icon>}
                          ...
                          {((subItem.note.playTime && subItem.note.playTime.indexOf('Note ') > -1) && (subItem.note.cubeType === 'Video' || subItem.note.cubeType === 'Audio')) && `${subItem.note.playTime}`}
                          <span className="date">{
