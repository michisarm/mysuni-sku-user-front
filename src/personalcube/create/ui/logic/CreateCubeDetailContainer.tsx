...
function CreateCubeDetailContainer() {
  ...
  const onSave = useCallback(async () => {
    ...
    if(result) {
      reactConfirm({
        title: getPolyglotText('저장완료', 'Create-DetailContentsButton-Complete'),
        message: getPolyglotText('저장되었습니다. 목록 페이지로 이동하시겠습니까?', 'Create-DetailContentsButton-CompleteInfo'),
        onOk: routeToCreateList
      });
    } else {
      reactAlert({ title: getPolyglotText('저장 실패', 'Create-DetailContentsButton-Fail'), message: getPolyglotText('저장을 실패했습니다. 잠시 후 다시 시도해주세요.', 'Create-DetailContentsButton-FailInfo') });
    }
  ...
  const onClickSave = useCallback(() => {
    ...
    if(blankField === 'none') {
      reactConfirm({
        title: getPolyglotText('저장 안내', 'Create-DetailContentsButton-ModalTitle'),
        message: getPolyglotText('입력하신 강좌를 저장 하시겠습니까?', 'Create-DetailContentsButton-ModalSubTitle'),
        onOk: onSave,
      });
    } else {
      ...
      <div className="add-personal-learning support">
        <div className="add-personal-learning-wrap">
          ...
          <div className="apl-notice">
            <PolyglotText defaultString="생성하신 학습에 대한 확인과 수정, 그리고 승인요청을 할수 있습니다." id="Create-SubTitle-Title" />
          </div>
        </div>
      </div>
                  ...
                  params.cubeState === 'OpenApproval' && (
                    <div className="buttons editor">
                      <Button className="fix bg" onClick={routeToCreateList}>
                        <PolyglotText defaultString="List" id="Create-DetailContentsButton-List" />
                      </Button>
                    </div>
                  )
                }
                  ...
                  params.cubeState !== 'OpenApproval' && (
                    <div className="buttons">
                      <Button className="fix line" onClick={routeToCreateList}>
                        <PolyglotText defaultString="Cancel" id="Create-DetailContentsButton-Cancel" />
                      </Button>
                      <Button className="fix line" onClick={onClickSave}>
                        <PolyglotText defaultString="Save" id="Create-DetailContentsButton-Save" />
                      </Button>
                    </div>
                  )
                }
