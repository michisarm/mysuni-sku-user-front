...
export default class EvaluationSheetModel extends DramaEntityObservableModel {
  ...
  constructor(evaluationSheet?: EvaluationSheetModel) {
    ...
    if (evaluationSheet) {
      ...
      this.answers = evaluationSheet.answers.map(
        (answer: AnswerModel) => new AnswerModel(answer)
      );
    }
  }
  ...
  static getNameValueList(evaluationSheet: EvaluationSheetModel) {
    ...
    nameValues.push({
      name: 'questionCount',
      value: evaluationSheet.questionCount,
    });
    nameValues.push({
      name: 'answers',
      value: JSON.stringify(evaluationSheet.answers),
    });
