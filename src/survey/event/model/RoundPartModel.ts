...
export default class RoundPartModel extends DramaEntityObservableModel {
  ...
  constructor(roundPartApiModel?: any) {
    ...
    if (roundPartApiModel) {
      ...
      this.descriptions = roundPartApiModel.descriptions && new LangStrings(roundPartApiModel.descriptions) || this.descriptions;
      this.period = new DatePeriod(roundPartApiModel.period);
  ...
  hasValidPeriod() {
    return this.period.startDate.length > 0
    && this.period.endDate.length > 0;
  }
