...
export function findBadgesByIds(badgeIds: string[]): Promise<Badge[] | undefined> {
  const axios = getAxios();
  const url = `${BASE_URL}/byIds`;
  const splitedIds = badgeIds && badgeIds.join(',') || [];

  return axios.get<Badge[]>(url, {
    params: {
      ids: splitedIds,
    },
  }).then(AxiosReturn);
...
export function findBadgesWithStudentCount(): Promise<BadgeWithBadgeStudentCountRom | undefined> {
  const axios = getAxios();
...
export function findAvailableBadgesByRdo(badgeRdo: BadgeRdo): Promise<OffsetElementList<Badge> | undefined> {
  const axios = getAxios();
  const url = `${BASE_URL}/available`;

  return axios.get<OffsetElementList<Badge>>(url, {
    params: badgeRdo
  }).then(AxiosReturn);
}

export function findBadgesByBadgeIssueState(myBadgeRdo: MyBadgeRdo): Promise<OffsetElementList<MyBadge> | undefined> {
  const axios = getAxios();
  const url = `${BASE_URL}/my`;

  return axios.get<OffsetElementList<MyBadge>>(url, {
    params: myBadgeRdo
  }).then(AxiosReturn);
}

export function challengeBadge(badgeStudentCdo: BadgeStudentCdo): Promise<boolean> {
  const axios = getAxios();
  const url = `${BASE_URL}/challenge`;

  return axios.post<boolean>(url, badgeStudentCdo).then(response => true).catch(error => false);
}

export function cancelBadgeChallenges(badgeStudentIds: string[]): Promise<boolean> {
  const axios = getAxios();
  const url = `${BASE_URL}/cancelChallenge`;

  return axios.put<boolean>(url, badgeStudentIds).then(response => true).catch(error => false);
}
...
export function requestIssue(badgeId: string): Promise<boolean> {
  ...
  return axios.put<boolean>(url).then(response => true).catch(error => false);
}
