...
const type: string[] = [
  getPolyglotText('오프라인', 'learning-learning-오프라인'),
  getPolyglotText('온라인', 'learning-learning-온라인'),
  getPolyglotText('상관없음', 'learning-learning-상관없음'),
];
const media: string[] = [
...
const time: string[] = [
  getPolyglotText('오전', 'learning-learning-am'),
  getPolyglotText('오후', 'learning-learning-pm'),
  getPolyglotText('상관없음', 'learning-learning-none'),
];
const goal: string[] = [
  getPolyglotText(
    '새로운 지식과 트렌드를 배우기 위해',
...
class FavoriteLearningTypeContainer extends Component<Props, State> {
  ...
  onSubmmit() {
    ...
    mediaGroup.map((media) =>
      learningTyps.idNames.push({ id: 'media', name: media })
    );
    goalGroup.map((goal) =>
      learningTyps.idNames.push({ id: 'goal', name: goal })
    );
    learningTyps.idNames.push({ id: 'etc', name: write });

    skProfileService.setAdditionalUserInfo(learningTyps);

    skProfileService
      .modifyStudySummary({
        nameValues: [
          {
            name: 'favoriteLearningTypes',
            value: JSON.stringify(learningTyps),
          },
        ],
      })
      .then(() => {
      ...
      <Form>
        <div className="type-check-wrap">
          ...
          <div className="type-check-box location">
            <h3 className="title-filter">
              <PolyglotText
                defaultString="온라인 학습 중 선호하는 유형은 무엇인가요?"
                id="learning-learning-제목2"
              />
            </h3>
            <div className="check-area">
              {media &&
          ...
          <div className="type-check-box time">
            <h3 className="title-filter">
              <PolyglotText
                defaultString="오프라인 학습 선호 시간대"
                id="learning-learning-제목3"
              />
            </h3>
            <div className="check-area">
              {time &&
          ...
          <div className="type-check-box purpose">
            <h3 className="title-filter">
              <PolyglotText
                defaultString="어떤 목적으로 교육을 수강하고 싶은가요?"
                id="learning-learning-제목4"
              />{' '}
              <span>
                <PolyglotText
                  defaultString="(중복 3개 선택 가능)"
                  id="learning-learning-옵션"
                />
              </span>
            </h3>
            <div className="check-area">
        ...
        <div className="button-area">
          <div className="error">
            <PolyglotText
              defaultString="학습형태 항목별 문의 사항에 대해 선택해주세요."
              id="learning-learning-주의"
            />
          </div>
