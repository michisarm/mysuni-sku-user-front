...
export default class CoursePlanApi {
  ...
  URL =
    ...
    process.env.REACT_APP_COURSE_PLAN_API === undefined ||
    process.env.REACT_APP_COURSE_PLAN_API === ''
      ? '/api/course/coursePlans'
      : process.env.REACT_APP_COURSE_PLAN_API;

  coursePlanContentsURL =
    process.env.REACT_APP_ENVIRONMENT === undefined ||
    process.env.REACT_APP_ENVIRONMENT === 'server' ||
    process.env.REACT_APP_COURSE_PLAN_CONTENT_API === undefined ||
    process.env.REACT_APP_COURSE_PLAN_CONTENT_API === ''
      ? '/api/course/coursePlanContents'
      : process.env.REACT_APP_COURSE_PLAN_CONTENT_API;
  ...
  findCoursePlan(coursePlanId: string) {
    //
    return axios
      .get<CoursePlanModel>(this.URL + `/${coursePlanId}`)
      .then(response => (response && response.data) || null);
  }
  ...
  findAllCoursePlan(offset: number = 0, limit: number = 20) {
    //
    return axios
      .get<OffsetElementList<CoursePlanModel>>(
  ...
  findAllCoursePlanByQuery(coursePlanRdo: CoursePlanRdoModel) {
    //
    return axios
      .get<OffsetElementList<CoursePlanModel>>(this.URL + `/searchKey`, {
        params: coursePlanRdo,
      })
      .then(response => (response && response.data) || null);
  }
  ...
  findCoursePlanContents(coursePlanContentsId: string) {
    //
    return axios
      .get<CoursePlanContentsModel>(
        this.coursePlanContentsURL + `/${coursePlanContentsId}`
      )
      .then(response => (response && response.data) || null);
  }
  ...
  findCoursePlanContentsV2(coursePlanContentsId: string) {
    ...
    return axios
      .get<CoursePlanContentsModel>(
        this.coursePlanContentsURL + `/v2/${coursePlanContentsId}`
      )
      .then(response => (response && response.data) || null);
  }

  modifyCoursePlanContents(
    coursePlanContentsId: string,
  ...
  findAllPrecedenceCourseList(coursePlanId: string) {
    return axios
      .get<OffsetElementList<CoursePlanModel>>(
        this.URL + `/precedenceCourseList/${coursePlanId}`
      )
      .then(response => (response && response.data) || null);
  }
  ...
  findAllPreCourseIdList(coursePlanId: string) {
    return axios
      .get<string[]>(this.URL + `/preCourseIdList/${coursePlanId}`)
      .then(response => (response && response.data) || null);
  }

  findAllCoursePlanInfo(coursePlanId: string, courseLectureId: string) {
    return axios
      .get<CoursePlanCustomModel>(
  ...
  findCollegeSortOrder(collegeId: string) {
    return axios
      .get<number>(
        this.coursesURL +
          `/arranges/count?collegeId=${collegeId}`
      )
