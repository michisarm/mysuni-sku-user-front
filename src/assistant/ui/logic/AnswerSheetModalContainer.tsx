                  ...
                  && questions.map(question => {
                    ...
                    return (
                      <List.Item as="li" key={question.questionNo}>
                        {numberOfTrials !== '0' && finichChkVal === 'Y' && answerChk === question.answer.replace('[', '').replace(']', '').replace('"', '').replace('"', '').replace('"', '').replace('"', '').replace('"', '').replace('"', '').replace('"', '').replace('"', '') && <div className="ol-title exact-answer" dangerouslySetInnerHTML={{__html:`${question.direction} <span className="q-score">(${question.allocatedPoint}점)</span>`}}/>
